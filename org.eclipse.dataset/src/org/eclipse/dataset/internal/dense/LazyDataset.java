...
import org.eclipse.dataset.metadata.OriginMetadataImpl;
import org.eclipse.dataset.metadata.Reshapeable;
import org.eclipse.dataset.metadata.Sliceable;
import org.eclipse.dataset.metadata.Transposable;
...
public class LazyDataset extends LazyDatasetBase implements Serializable, Cloneable {
	...
	public LazyDataset(String name, int dtype, int elements, int[] shape, ILazyLoader loader) {
		...
		this.isize = elements;
		try {
			size = DatasetUtils.calculateLongSize(shape);
	...
	public LazyDataset squeezeEnds() {
		setShapeInternal(DatasetUtils.squeezeShape(shape, true));
		return this;
	}
	...
	private void setShapeInternal(int... nShape) {
		...
		long nsize = DatasetUtils.calculateLongSize(nShape);
		if (nsize != size) {
	...
	public LazyDataset getSliceView(SliceND slice) {
		...
		view.shape = nShape;
		view.size = DatasetUtils.calculateLongSize(nShape);
		if (begSlice == null) {
	...
	public static int getMaxSliceLength(ILazyDataset lazySet, int dimension) {
		...
		final double size = DTypeUtils.getItemsize(DTypeUtils.getDTypeFromClass(lazySet.getElementClass()), lazySet.getElementsPerItem());
