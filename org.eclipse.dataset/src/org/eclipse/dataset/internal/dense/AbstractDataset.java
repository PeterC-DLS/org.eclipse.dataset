...
import org.eclipse.dataset.dense.ContiguousIteratorWithPosition;
...
import org.eclipse.dataset.dense.NullIterator;
...
public abstract class AbstractDataset extends LazyDatasetBase implements Dataset {
	...
	public IndexIterator getSliceIterator(SliceND slice) {
		if (DatasetUtils.calculateLongSize(slice.getShape()) == 0) {
	...
	public int getItemsize() {
		return DTypeUtils.getItemsize(getDType(), getElementsPerItem());
	}
...
//		}
	...
	public int checkAxis(int axis) {
		return DatasetUtils.checkAxis(shape.length, axis);
	}
	...
	public boolean isCompatibleWith(final ILazyDataset g) {
		return DatasetUtils.areShapesCompatible(shape, g.getShape());
	}
	...
	public static String storeName(boolean ignoreNaNs, String name) {
	 ...
	 */
	public abstract void setItemDirect(final int dindex, final int sindex, final Object src);
