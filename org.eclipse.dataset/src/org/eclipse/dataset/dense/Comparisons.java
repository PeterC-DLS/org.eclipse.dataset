...
public class Comparisons {
	...
	public static BooleanDataset equalTo(Object a, Object b, BooleanDataset o) {
		...
		final BooleanDataset r = o == null ? (BooleanDataset) DatasetFactory.zeros(sl.get(0), Dataset.BOOL): o;
	...
	public static BooleanDataset allTrue(IDataset a, int axis) {
		axis = DatasetUtils.checkAxis(a.getRank(), axis);
		...
		BooleanDataset result = (BooleanDataset) DatasetFactory.zeros(nshape, Dataset.BOOL);
	 ...
	 */
	public static List<Dataset> nonZero(Dataset a) {
		final int rank = a.getRank();
		final List<List<Integer>> indices = new ArrayList<List<Integer>>();
		List<Dataset> indexList = new ArrayList<Dataset>();
		...
		if (length > 0 ) {
			for (int j = 0; j < rank; j++) {
				indexList.add(DatasetFactory.createFromList(indices.get(j)));
			}
		}
		return indexList;
	}
