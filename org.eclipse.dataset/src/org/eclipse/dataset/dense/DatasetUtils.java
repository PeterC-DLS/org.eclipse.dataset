...
public class DatasetUtils {
	...
	public static Dataset concatenate(final IDataset[] as, final int axis) {
		...
		for (; i < anum; i++) {
			...
			if (!DatasetUtils.areShapesCompatible(ashape, as[i].getShape(), axis)) {
	...
	public static Dataset select(BooleanDataset[] conditions, Object[] choices, Object def) {
		...
		for (Object a : choices) {
			...
			if (a instanceof Dataset) {
				...
				t = DTypeUtils.getDTypeFromObject(a);
				s = 1;
			}
			if (t > dt)
				dt = t;
	 ...
	 */
	public static List<IntegerDataset> calculatePositionsFromIndexes(Dataset indices, int[] shape) {
		int rank = shape.length;
	 ...
	 */
	public static IntegerDataset calculateIndexesFromPositions(List<? extends Dataset> positions, int[] shape, int... mode) {
		int rank = shape.length;
	 ...
	 * @return size
	 */
	public static long calculateLongSize(final int[] shape) {
		double dsize = 1.0;
	...
	private static boolean getShapeFromObj(final ArrayList<Integer> ldims, Object obj, int depth) {
		...
		if (ca != null) {
			...
			for (int i = 0; i < s.length; i++) {
				DatasetUtils.updateShape(ldims, depth++, s[i]);
			}
			return true;
		} else {
	...
	public static int[] squeezeShape(final int[] oshape, int axis) {
		...
		if (axis < 0 || axis >= rank) {
			utilsLogger.error("Axis argument is outside allowed range");
